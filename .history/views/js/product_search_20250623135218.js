...
$(document).ready(function () {
  ...
  $(document).on("click", ".remove-pinned-product", function () {
    ...
    Swal.fire({
      title: "Are you sure?",
      text: "Are you sure you want to remove this pinned product?",
      icon: "warning",
      ...
      if (result.isConfirmed) {
        $.ajax({
          ...
          error: function () {
            showErrorMessage(
              "An error occurred while removing the pinned product."
            );
          },
        });
      }
...
$(document).ready(function () {
  $(document).on("click", ".list-action-enable", function (e) {
    ...
    // AJAX call
    $.post(
      `index.php?controller=AdminMerchandising&ajax=1&action=updatePublishStatus&token=${token}`,
      {
    ...
    return false; // 👈 Yeh bhi zaruri hai to 100% stop default behavior
  });
});
