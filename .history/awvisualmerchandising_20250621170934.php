...
class Awvisualmerchandising extends Module
{
    public $tabs = [
        [
            'route_name' => 'awvisualmerchandising_index',  // 👈 Symfony route name
            'class_name' => 'AdminAwvisualmerchandising',   // 👈 Internal legacy class
            'visible' => true,
            'name' => [
                'en' => 'Visual Merchandising',
                'fr' => 'Marchandisage Visuel'
            ],
            'parent_class_name' => 'AdminParentThemes', // 👈 Under Appearance > Theme
            'wording' => 'Visual Merchandising',
            'wording_domain' => 'Modules.Awvisualmerchandising.Admin'
        ],
    ];
    ...
    public function uninstall()
    {
        ...
        $id_tab = (int) Db::getInstance()->getValue('SELECT id_tab FROM ' . _DB_PREFIX_ . 'tab WHERE class_name = "AdminMerchandising"');
        if ($id_tab) {
            $tab = new Tab($id_tab);
            $tab->delete();
        }
    ...
    private function registerTab()
    {
        ...
        $tab->id_parent = (int) Db::getInstance()->getValue('SELECT id_tab FROM ' . _DB_PREFIX_ . 'tab WHERE class_name = "AdminCatalog"');
    ...
    public function getCategoryProducts($id_category, $id_awvisualmerchandising)
    {
        ...
        $allProductsQuery = "SELECT DISTINCT
            p.id_product,
            p.date_add,
            pl.name,
            p.reference AS sku,
        ...
        LEFT JOIN {$prefix}product_lang pl
            ON p.id_product = pl.id_product AND pl.id_lang = {$langId}
        LEFT JOIN {$prefix}manufacturer m
            ON p.id_manufacturer = m.id_manufacturer
        LEFT JOIN {$prefix}image i
            ON p.id_product = i.id_product AND i.cover = 1
        LEFT JOIN {$prefix}product_supplier ps
            ON p.id_product = ps.id_product
        LEFT JOIN {$prefix}supplier s
        ...
        LEFT JOIN {$prefix}attribute_lang al
            ON pac.id_attribute = al.id_attribute AND al.id_lang = {$langId}
        LEFT JOIN {$prefix}feature_product fp
            ON p.id_product = fp.id_product
        LEFT JOIN {$prefix}feature_value_lang fvl
            ON fp.id_feature_value = fvl.id_feature_value AND fvl.id_lang = {$langId}
        LEFT JOIN {$prefix}specific_price pd
        ...
        AND p.id_product NOT IN (
            SELECT id_product FROM {$prefix}awvisualmerchandising_pinned_product
            WHERE id_awvisualmerchandising = {$id_awvisualmerchandising}
            UNION
            SELECT id_product FROM {$prefix}awvisualmerchandising_hidden_product
            WHERE id_awvisualmerchandising = {$id_awvisualmerchandising}
        )";
    ...
    public function processAndSortBoostedProducts($data, $totalCount)
    {
        ...
        foreach ($grouped as $percentage => $items) {
            ...
            $limit = (int) round(($percentage / 100) * $totalCount);
